...
def register():
    ...
    db.session.flush()  # supaya new_user.id tersedia sebelum commit

    # Auto-create UserBab untuk semua bab
    for bab in Bab.query.order_by(Bab.id).all():
        db.session.add(UserBab(
            userid=new_user.id,
            babid=bab.id,
            locked=(bab.id != 1)
        ))

    # Auto-create UserMateri untuk semua materi
    for materi in Materi.query.all():
        db.session.add(UserMateri(
            userid=new_user.id,
            materiid=materi.id,
        ))
